...
export class FormsComponent {
  ...
  filterUnits(unit: Location, open_hour:string, close_hour:string){
    ...
    let todays_week_day = this.transformWeekday(new Date().getDay());

    for(let i = 0; i < unit.schedules.length; i++){
      let schedule_hour = unit.schedules[i].hour
      let schedule_weekday = unit.schedules[i].weekdays
      if(todays_week_day === schedule_weekday){
  ...
  onSubmit():void{
    ...
    if(this.formGroup.value.hour){
      const OPEN_HOUR = OPENNING_HOURS[this.formGroup.value.hour as HOUR_INDEX].first
      const CLOSE_HOUR = OPENNING_HOURS[this.formGroup.value.hour as HOUR_INDEX].last
